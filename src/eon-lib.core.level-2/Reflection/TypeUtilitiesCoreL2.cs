...
namespace Eon.Reflection {
	...
	public static class TypeUtilitiesCoreL2 {
		...
		public static Type GetType(string typeName, bool throwOnError) {
			...
			if (__TypeByNameRepositorySpinLock.Invoke(() => __TypeByNameRepository.TryGetValue(typeName, out result)))
				return result;
			...
			try {
				...
				result = null;
				platformGetTypeException = firstException;
			}
			if (platformGetTypeException == null)
				return P_GetOrAddIntoTypeByNameRepository(typeName: typeName, type: result);
			//
			Type returnResult(Exception locError, string locTypeName, Type locType, bool locThrowOnError1) {
				if (locError is null)
					return locType;
				else if (locThrowOnError1)
					throw new EonException(message: $"Ошибка при попытке получить тип по его имени '{locTypeName}'.", innerException: locError);
				else
					return null;
			}
			//
			var assemblyNamePartStartIndex = typeName.IndexOf(TypeUtilities.TypeAssemblyQualifiedNamePartDelimiter);
			...
			if (assemblyNamePartStartIndex < 1 || assemblyNamePartStartIndex == typeName.Length - 1 || string.IsNullOrEmpty((assemblyNamePart = typeName.Substring(assemblyNamePartStartIndex + 1).Trim())))
				return returnResult(locError: platformGetTypeException, locTypeName: typeName, locType: null, locThrowOnError1: throwOnError);
			...
			var matchAssemblies =
				...
				.CurrentDomain
				.GetAssemblies()
				.Where(locItem => string.Equals((loadedAssemblyName = new AssemblyName(locItem.FullName)).Name, assemblyName.Name, StringComparison.OrdinalIgnoreCase) && loadedAssemblyName.GetCultureName().EqualsOrdinalCS(assemblyName.GetCultureName()) && loadedAssemblyName.GetPublicKeyToken().IsEqual(array2: assemblyName.GetPublicKeyToken()))
				.Take(2)
				.ToArray();
			if (matchAssemblies.Length != 1)
				...
				try {
					result = matchAssemblies[ 0 ].GetType(typeName.Substring(0, assemblyNamePartStartIndex), true);
				}
				catch (Exception exception) {
					return returnResult(locError: exception, locTypeName: typeName, locType: null, locThrowOnError1: throwOnError);
				}
				return P_GetOrAddIntoTypeByNameRepository(typeName, result);
			}
		}
