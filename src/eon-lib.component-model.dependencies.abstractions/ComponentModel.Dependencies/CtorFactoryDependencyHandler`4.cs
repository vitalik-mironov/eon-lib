...
namespace Eon.ComponentModel.Dependencies {
	...
	public class CtorFactoryDependencyHandler<TArg1, TArg2, TArg3, TDependencyInstance>
		:FactoryDependencyHandlerBase<TDependencyInstance>
		where TDependencyInstance : class {

		const int __ArgsCount = 3;

		#region Static members

		static readonly DisposableLazy<Func<TArg1, TArg2, TArg3, TDependencyInstance>> __DependencyInstanceCtor;
		...
		Func<Func<TArg1, TArg2, TArg3, TDependencyInstance>> _getDependencyCtor;

		Func<TArg1, TArg2, TArg3, TDependencyInstance> _customCtor;

		readonly bool _canShareDependency;

		IArgsTuple<TArg1, TArg2, TArg3> _factoryArgs;
		...
		public CtorFactoryDependencyHandler(
			...
			IArgsTuple<TArg1, TArg2, TArg3> factoryArgs = null)
			: this(
				 customCtor: null,
		...
		public CtorFactoryDependencyHandler(
			Func<TArg1, TArg2, TArg3, TDependencyInstance> customCtor = null,
		...
		protected override Task<DependencyResult<TDependencyInstance>> ResolveNewDependencyInstanceAsync(IDependencyResolutionContext context) {
			try {
				...
				if (factoryArgs == null) {
					if ((factoryArgs = specs.NewInstanceFactoryArgs as IArgsTuple<TArg1, TArg2, TArg3>) == null || factoryArgs.ArgsCount != __ArgsCount)
						...
						result:
							new DependencyResult<TDependencyInstance>(
								instance:
									...
									()
									(factoryArgs.Arg1, factoryArgs.Arg2, factoryArgs.Arg3),
								isNewInstance: true));
			}
