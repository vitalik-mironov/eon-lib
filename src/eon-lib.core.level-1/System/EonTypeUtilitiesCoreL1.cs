...
namespace System {
	...
	public static class EonTypeUtilitiesCoreL1 {
		...
		//
		public static bool IsGenericTypeInterfaceImplementedBy(this Type genericDefinition, Type implementation, out Type[ ] implementationGenericArgs) {
			...
			var result = false;
			implementationGenericArgs = null;
			for (var i = 0; i < implementedInterfaces.Length; i++) {
				...
				if (result) {
					implementationGenericArgs = implementedInterfaces[ i ].GetGenericArguments();
					break;
				}
			}
			return result;
